...
import io.ourfit.api.global.security.data.AdminAuditEvent;
...
public class AdminApiAuthorizationFilter extends AbstractSecurityFilter {
  ...
  private boolean checkAdminPrivileges(Authentication authentication, AdminApi adminApi) {
    if (authentication.getPrincipal() instanceof OurfitUserDetails userDetails) {
  ...
  private void publishAuditEvent(
    ...
    final long userId = this.auditorAware.getCurrentAuditorOrAnonymous();
